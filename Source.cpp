...
int OutputNormals(std::vector<OpenMesh_Mesh::Normal> subsample_points_normals)
{
...
int DijkstraAlgorithm(OpenMesh_Mesh mesh, std::vector<OpenMesh_Mesh::Point> subsampled_points, OpenMesh_Mesh::Point start_point, std::vector<std::vector<double>> *distance)
{
	...
	int count = 0;
	OpenMesh_Mesh::VertexHandle s, t;
	...
	for (OpenMesh_Mesh::VertexIter it = mesh.vertices_begin(); it != mesh.vertices_end(); ++it)
	{
	...
	mesh.update_normals();
	std::vector<OpenMesh_Mesh::Normal> subsample_points_normals;
...
std::vector<OpenMesh_Mesh::Point> ReadSubsampledPoints()
{
	std::vector<OpenMesh_Mesh::Point> subsampled_points;
	...
	do
	{
		...
		if (std::sscanf(a, "%lf%lf%lf", &x, &y, &z) == 3)
		{
			...
			subsampled_points.push_back(OpenMesh_Mesh::Point(x, y, z));
		}
		count++;
	} while (!in.eof());
...
int OutputAsTXT(std::vector<std::vector<double>> distance)
{
	std::ofstream outdata;
	std::string name_file = "C:\\Users\\sinara\\Desktop\\distance.txt";
	outdata.open(name_file, std::ios::out);
	outdata.clear();
...
int main(int argc, char *argv[])
{
	...
	std::vector<OpenMesh_Mesh::Point> subsampled_points = ReadSubsampledPoints();

	std::vector<std::vector<double>> distance;
	DijkstraAlgorithm(mesh, subsampled_points, subsampled_points[0], &distance);

	OutputAsTXT(distance);
